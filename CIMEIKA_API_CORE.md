...
```json
{
  "persona": "ci",
  "mode": "echo-core",
  "reply": "[ci] Прийнято. Останнє повідомлення користувача: Привіт, Ci.",
  "last_user": "Привіт, Ci."
}
```
...
- `ci` — центр, інтегратор (головна персона для OpenAI Apps).
- інші модулі — статусні, готові до розширення маршрутизаторами та логікою.

Цей документ фіксує поточний стан ядра як `1` (константа) для репозиторію `cimeika-core-api`.
